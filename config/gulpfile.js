...
// * constent in case at different points in the compilation cycle we want to regenerate the docs. (RA)
const generateDocs = () => {
    let entryPaths = Object.values(config.entry).slice().map(entry => { return (entry.slice().pop() ?? "")}); // Removes special entry points from WebPack
    return documentation.build(entryPaths, {shallow: true})
        .then(documentation.formats.html)
        .then(output => {
            streamArray(output).pipe(vfs.dest(paths.docs));
        })
        .catch(function(err) {
            console.log(err);
        });
