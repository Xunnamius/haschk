...
// Constant in case at different points in the compilation cycle we want to regenerate the docs. (RA)
const generateDocs = () => {
    // Removes special entry points from WebPack
    let entryPaths = Object.values(config.entry).slice().map(entry => { return (entry.slice().pop() ?? "")});
